...
export default function NewIpKitClient() {
  const [isLoading, setIsLoading] = useState(false)
  const router = useRouter()

  // Get brand permissions and user brands
  const { userBrands, loading: permissionsLoading, refreshPermissions } = useBrandPermissions()
  const currentBrand = userBrands[0] // Use first brand for now

  // Listen for brand creation via window events
  useEffect(() => {
    const handleBrandCreated = () => {
      refreshPermissions()
    }
    
    window.addEventListener('brandCreated', handleBrandCreated)
    
    return () => {
      window.removeEventListener('brandCreated', handleBrandCreated)
    }
  }, [refreshPermissions])
  ...
  if (!currentBrand) {
    // Show simple message directing to dashboard
    return (
      <div className="container mx-auto py-8 max-w-4xl">
        <div className="text-center py-8">
          ...
          <p className="text-muted-foreground mb-4">
            To create IP kits, you need to set up your brand first. Go to your dashboard to get started.
          </p>
