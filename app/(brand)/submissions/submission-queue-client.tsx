...
export function SubmissionQueueClient({ 
  ...
  const toggleAllSubmissions = () => {
    ...
    const allPendingSelected = pendingSubmissions.every(s => selectedSubmissions.includes(s.id))
    
    if (allPendingSelected) {
      // Deselect all pending submissions
      const newSelection = selectedSubmissions.filter(id => 
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="sm" asChild>
                      <Link href={`/brand/submissions/${submission.id}`}>
              ...
              <CardContent className="pt-0 space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-muted-foreground">
                    <span className="font-medium">{submission.brand?.name}</span> • {submission.campaign?.title}
                  </div>
