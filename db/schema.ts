...
export const assets = pgTable(
  "assets",
  {
    ...
    ipKitId: uuid("ip_kit_id").references(() => ipKits.id, {
      onDelete: "cascade",
    }), // Now nullable for campaign assets
...
export const assetIpKits = pgTable(
  "asset_ip_kits",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    assetId: uuid("asset_id")
      ...
      .notNull(),
    ipKitId: uuid("ip_kit_id")
      .references(() => ipKits.id, {onDelete: "cascade"})
      .notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  table => ({
...
export const submissions = pgTable(
  "submissions",
  {
    ...
    ipKitId: uuid("ip_kit_id").references(() => ipKits.id, {
      onDelete: "set null",
    }), // Reference to IP Kit used for this submission
    storyProtocolIpId: text("story_protocol_ip_id"), // Story Protocol blockchain IP Asset ID (0x address)
  ...
  table => ({
    ...
    ipKitIdIdx: index("submissions_ip_kit_id_idx").on(table.ipKitId),
    storyProtocolIpIdIdx: index("submissions_story_protocol_ip_id_idx").on(
      table.storyProtocolIpId
    ),
...
export const submissionAssets = pgTable(
  ...
  table => ({
    submissionIdIdx: index("submission_assets_submission_id_idx").on(
      table.submissionId
    ),
...
export const submissionAssetsRelations = relations(
  submissionAssets,
  ({one}) => ({
    submission: one(submissions, {
